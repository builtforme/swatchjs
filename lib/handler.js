...
function getSchemaArgsMetadata(methodSchema) {
  ...
  // Return an array (ordered) of arguments and their metadata
  return schemaArgs.map((arg, idx) => {
...
function getMatchFn(methodSchema) {
  // Build a structured metadata list for arguments
  const argsMetadata = getSchemaArgsMetadata(methodSchema);
...
function buildMethodDocs(methodSchema) {
  // Build a structured metadata list for arguments
  const argsMetadata = getSchemaArgsMetadata(methodSchema);

  const args = argsMetadata.map(arg => ({
    description: arg.description,
    name: arg.name,
    type: arg.type,
  }));
...
function handler(methodSchema) {
  ...
  const methodDocs = buildMethodDocs(normalizedSchema);
